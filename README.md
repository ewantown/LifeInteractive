...
### Intention:
The game ought to be fun, of course, but the program was/is a sort of "sandbox" for myself, so there a number of boolean "toggles" in the code; e.g. toggling 'devMode' will add a "surprise" button to the menus, which runs some additional code. The general idea may eventually be of use to me as a testing ground for simple game-playing agents and/or multi-agent systems; e.g. have another agent interact with the board by selecting a single cell per generation to "flip", as decided by search of a game tree, then automate the tank(s) to play optimally against such an agent... one day. Hopefully it is of interest to someone else, for some reason. 
